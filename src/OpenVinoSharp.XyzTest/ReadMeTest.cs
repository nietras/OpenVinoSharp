...
namespace OpenVinoSharp.XyzTest;
...
public partial class ReadMeTest
{
    ...
    public void ReadMeTest_UpdateBenchmarksInMarkdown()
    {
        ...
        {
            //{ "BENCHNAME.md", new("BENCHNAME Benchmark Results", "##### BENCHNAME Benchmark Results", "## Example Catalogue", "###### ") },
        };

        var benchmarksDirectory = Path.Combine(s_rootDirectory, "benchmarks");
        if (!Directory.Exists(benchmarksDirectory)) { Assert.Inconclusive($"No '{benchmarksDirectory}' found"); }
    ...
    {
        var publicApi = typeof(Ov).Assembly.GeneratePublicApi();
